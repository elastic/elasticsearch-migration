...

   {
     ...
     check : function(field) {
       if ("type" in field && field.type === "boolean") {
         return "Boolean field `" + field._name + "` will return `1/0` instead of `T/F` in scripts, aggregations, or sort values"
       }
     }
   },

   {
     ...
     check : function(field) {
       if ("postings_format" in field) {
         return "Field `" + field._name + "` contains a `posting_format` which is no longer supported"
       }
     }
   },
